...
@onready var _rename_button : Button = \
	$VBoxContainer/Main/LeftPane/StringListActions/RenameButton
...
func _ready():
	...
	_file_menu.get_popup().connect("id_pressed", _on_FileMenu_id_pressed)
	
	_edit_menu.get_popup().connect("id_pressed", _on_EditMenu_id_pressed)
...
func _setup_dialogs():
	...
	_open_dialog.title = "Open translations"
	_open_dialog.add_filter("*.csv ; CSV files")
	_open_dialog.add_filter("*.po ; Gettext files")
	_open_dialog.file_mode = FileDialog.FILE_MODE_OPEN_FILE
	_open_dialog.file_selected.connect(_on_OpenDialog_file_selected)
	_add_dialog(_open_dialog)

	_save_file_dialog = FileDialog.new()
	_save_file_dialog.title = "Save translations as CSV"
	_save_file_dialog.add_filter("*.csv ; CSV files")
	_save_file_dialog.file_mode = FileDialog.FILE_MODE_SAVE_FILE
	_save_file_dialog.file_selected.connect(_on_SaveFileDialog_file_selected)
	_add_dialog(_save_file_dialog)

	_save_folder_dialog = FileDialog.new()
	_save_folder_dialog.title = "Save translations as gettext .po files"
	_save_folder_dialog.file_mode = FileDialog.FILE_MODE_OPEN_DIR
	_save_folder_dialog.dir_selected.connect(_on_SaveFolderDialog_dir_selected)
	_add_dialog(_save_folder_dialog)
	
	_string_edit_dialog = StringEditionDialogScene.instantiate()
	_string_edit_dialog.set_validator(_validate_new_string_id)
	_string_edit_dialog.submitted.connect(_on_StringEditionDialog_submitted)
	_string_edit_dialog.hide()
	_add_dialog(_string_edit_dialog)
	
	_language_selection_dialog = LanguageSelectionDialogScene.instantiate()
	_language_selection_dialog.language_selected.connect(_on_LanguageSelectionDialog_language_selected)
	_language_selection_dialog.hide()
	...
	_remove_language_confirmation_dialog.confirmed.connect(_on_RemoveLanguageConfirmationDialog_confirmed)
	_add_dialog(_remove_language_confirmation_dialog)
	
	_extractor_dialog = ExtractorDialogScene.instantiate()
	_extractor_dialog.set_registered_string_filter(_is_string_registered)
	_extractor_dialog.import_selected.connect(_on_ExtractorDialog_import_selected)
	_extractor_dialog.hide()
	_add_dialog(_extractor_dialog)
	
	_remove_string_confirmation_dialog = ConfirmationDialog.new()
	_remove_string_confirmation_dialog.confirmed.connect(_on_RemoveStringConfirmationDialog_confirmed)
...
func _on_FileMenu_id_pressed(id: int):
	match id:
		...
		MENU_FILE_REMOVE_LANGUAGE:
			...
			_remove_language_confirmation_dialog.title = \
				str("Remove language `", language, "`")
			_remove_language_confirmation_dialog.popup_centered()
		
		MENU_FILE_EXTRACT:
			_extractor_dialog.popup_centered()
...
func _create_translation_edit(language: String):
	...
	edit.connect("text_changed", _on_TranslationEdit_text_changed.bind(language))
...
func _set_language_tab_title(language: String, title: String):
	...
	for i in _translation_tab_container.get_child_count():
		if _translation_tab_container.get_child(i) == page:
			...
			_translation_tab_container.queue_redraw()
			return
...
func _on_StringList_multi_selected(index: int, selected: bool):
	var selection_count := len(_string_list.get_selected_items())
	if selection_count > 1:
		#TODO: multiselect behaviour
		for language in _languages:
...
func _on_RemoveButton_pressed():
	...
	if len(selected_items) == 0:
		...
		_remove_string_confirmation_dialog.title = str("Remove `", str_id, "`")
		_remove_string_confirmation_dialog.dialog_text = \
			"Do you really want to remove this string (`" + str_id + "`) and all its translations? (There is no undo)"
		_remove_string_confirmation_dialog.popup_centered()
	else:
		_remove_string_confirmation_dialog.title = str("Remove all selected items")
		_remove_string_confirmation_dialog.dialog_text = \
			"Do you really want to remove these strings and all their translations? (There is no undo)"
		_remove_string_confirmation_dialog.popup_centered()
...
func _on_RemoveStringConfirmationDialog_confirmed():
	...
	if len(selected_items) == 0:
		...
		for i in range(len(selected_items) - 1, -1, -1):
			var strid := _string_list.get_item_text(selected_items[i])
			_string_list.remove_item(selected_items[i])
			_data.erase(strid)
			for language in _languages:
				_set_language_modified(language)
...
func _on_RenameButton_pressed():
	...
	if len(selected_items) == 0 or len(selected_items) > 1:
		_logger.error("Tried to rename more or fewer than one item??")
		return
	var str_id := _string_list.get_item_text(selected_items[0])
...
func _on_StringList_item_activated(index: int):
